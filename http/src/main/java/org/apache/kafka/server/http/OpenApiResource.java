 ...
 *
 * <p>Both paths are allow-listed in {@link BasicAuthFilter} so the spec and UI
 * load without credentials. "Try it out" in the UI will prompt for Basic Auth
 * and send it on to the actual {@code /v1/topics/...} request.</p>
 */
@Path("/")
public class OpenApiResource {
    ...
    private Response streamResource(String name, String contentType) {
        try (InputStream stream = getClass().getResourceAsStream(name)) {
            ...
            return Response.ok(stream.readAllBytes()).type(contentType).build();
        } catch (IOException e) {
