 ...
 * Everything the REST proxy needs arrives in here; nothing else is allowed,
 * which keeps the SPI seam narrow.
 *
 ...
 * @param brokerConfig           broker configuration — {@code :http} reads per-listener
 *                               SSL overrides via {@code valuesWithPrefixOverride}
 * @param reconfigurableRegistry hook for registering the HTTPS {@code SslContextFactory}
 *                               with {@code DynamicBrokerConfig} so cert rotation works
