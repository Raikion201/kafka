...
public final class AcuitySchedulingSourceTask extends SourceTask {
  private static final ObjectMapper MAPPER = new ObjectMapper();

  private AcuitySchedulingConnectorConfig config;
  ...
  public void start(Map<String, String> props) {
    this.config = new AcuitySchedulingConnectorConfig(props);
  ...
  public List<SourceRecord> poll() throws InterruptedException {
    ...
    all.addAll(pollAppointments());
    return all;
  }

  private List<SourceRecord> pollAppointments() throws InterruptedException {
    final String streamName = "appointments";
    List<SourceRecord> result = new ArrayList<>();
    StringBuilder urlBuilder = new StringBuilder("https://acuityscheduling.com/api/v1/appointments");
    urlBuilder.append("?minDate=" + URLEncoder.encode(config.getStartDate(), StandardCharsets.UTF_8));
    urlBuilder.append("&maxDate=" + URLEncoder.encode(Instant.now().toString(), StandardCharsets.UTF_8));
    try {
